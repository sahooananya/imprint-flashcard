...
export default function MotivationBubble({ trigger = 'load' }) {
  ...
  return (
    <div onClick={dismiss} style={{
      ...
      cursor: 'pointer',
      animation: exiting ? 'bubbleOut 0.4s ease forwards' : 'bubbleIn 0.35s ease forwards',
      ...
      <style>{`
        ...
        @keyframes bubbleOut { from { opacity: 1; transform: translateY(0); } to { opacity: 0; transform: translateY(8px); } }
      `}</style>
    </div>
  );
}
